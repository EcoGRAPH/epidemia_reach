...
//
var woredas = ee.FeatureCollection("users/dawneko/public/Eth_Admin_Woreda_2019_20200702"),
    //create region outer boundary to filter products on
    ethiopia = woredas.geometry().bounds(),
...
// // Update woredas Feature Collection with area calculation added
// var addArea = function(feature){
//   return feature.set({'area_woreda': feature.geometry().area()});
// };
// woredas = woredas.map(addArea); 
...
function calculateEnvVars(userStartDate, userEndDate){
  ...
  function rescaleLst(image) {
    ...
    // //area of good pixels. May not be working correctly, check if incorrectly same value                  
    // var lstdBinary = lst_day.unmask(0).neq(0); 
    // var lstdArea = lstdBinary.multiply(ee.Image.pixelArea()).rename('area_lstd');
    // var lstnBinary = lst_night.unmask(0).neq(0);
    // var lstnArea = lstnBinary.multiply(ee.Image.pixelArea()).rename('area_lstn');
    // var lstmBinary = lst_mean.unmask(0).neq(0);
    // var lstmArea = lstmBinary.multiply(ee.Image.pixelArea()).rename('area_lstm');
  ...
  function calcDailyLst(curdate) {
      ...
      var lst_mean = lstVars
                       ...
                       .first()
                       .rename('lst_mean');   
      // var lstdArea = lstVars
      //                 .select('area_lstd')
      //                 .filterDate(moddate, moddate.advance(1, 'day'))
      //                 .first()
      //                 .rename('area_lstd');   
      // var lstnArea = lstVars
      //                 .select('area_lstn')
      //                 .filterDate(moddate, moddate.advance(1, 'day'))
      //                 .first()
      //                 .rename('area_lstn');   
      // var lstmArea = lstVars
      //                 .select('area_lstm')
      //                 .filterDate(moddate, moddate.advance(1, 'day'))
      //                 .first()
      //                 .rename('area_lstm');   
  ...
  function sumZonalLst(image) { 
        ...
        .combine({
          ...
          .combine({
            ...
            .combine({
              ...
              // outputPrefix: 'area_lstd'})
              // .combine({
              //   reducer2: ee.Reducer.sum(),
              //   outputPrefix: 'area_lstn'})
              //   .combine({
              //     reducer2: ee.Reducer.sum(),
              //     outputPrefix: 'area_lstm'})
              //     .combine({
    ...
    var output = image2
    .select(['doy', 'year', 'lst_day', 'lst_night', 'lst_mean', 'lst_day', 'lst_night', 'lst_mean'], 
            ['doy', 'year', 'lst_day', 'lst_night', 'lst_mean', 'dayToCount', 'nightToCount', 'meanToCount'])
    // .select(['doy', 'year', 'lst_day', 'lst_night', 'lst_mean', 'area_lstd', 'area_lstn', 'area_lstm', 'lst_day', 'lst_night', 'lst_mean'], 
    //         ['doy', 'year', 'lst_day', 'lst_night', 'lst_mean', 'area_lstd', 'area_lstn', 'area_lstm', 'dayToCount', 'nightToCount', 'meanToCount'])
    .reduceRegions({
           collection: woredas,
           reducer: reducers,
           scale: 1000}); 
  ...
  // Flatten the results for export
  var lstNamesOld = ['NewPCODE', 'R_NAME', 'Z_NAME', 'W_NAME', 'yearmean', 'mean', 
                      'lst_daymean', 'lst_nightmean', 'lst_meanmean', 
                      'pixels_lstdcount', 'pixels_lstncount', 'pixels_lstmcount', 'pixels_totalcount']; 
  var lstNamesNew = ['NewPCODE', 'R_NAME', 'Z_NAME', 'W_NAME', 'year', 'doy', 
  ...
  //                     'lst_daymean', 'lst_nightmean', 'lst_meanmean', 
  //                     'pixels_lstdcount', 'pixels_lstncount', 'pixels_lstmcount', 'pixels_totalcount', 
  //                     'area_lstdsum', 'area_lstdsum', 'area_lstdsum', 'area_woreda'];
  // var lstNamesNew = ['NewPCODE', 'R_NAME', 'Z_NAME', 'W_NAME', 'year', 'doy', 
  //                     'lst_day', 'lst_night', 'lst_mean', 
  //                     'pixels_lstd', 'pixels_lstn', 'pixels_lstm', 'pixels_total', 
  //                     'area_lstd', 'area_lstn', 'area_lstm', 'area_woreda'];
  ...
  function calcBrdfIndices(image) {
    ...
    var ndwi6 = image.normalizedDifference(['nir_r', 'swir2_r'])
                    .rename('ndwi6');
    // //area of good pixels of the NDVI index                 
    // var ndviBinary = ndvi.unmask(0).neq(0); 
    // var ndviArea = ndviBinary.multiply(ee.Image.pixelArea()).rename('area_ndvi');
  ...
  function calcDailyBrdf(curdate) {
      ...
      var outimg = ee.Image(ee.Algorithms.If(brdfSize.eq(0),
                                               ee.Image.constant(0).selfMask() //ndvi
                                                 .addBands(ee.Image.constant(0).selfMask()) //evi
                                                 .addBands(ee.Image.constant(0).selfMask()) //savi
                                                 .addBands(ee.Image.constant(0).selfMask()) //ndwi5
                                                 .addBands(ee.Image.constant(0).selfMask()) //ndwi6
                                                // .addBands(ee.Image.constant(0).selfMask()) //area_ndvi
                                                 .rename(['ndvi', 'evi', 'savi', 'ndwi5', 'ndwi6'])
                                                 //.rename(['ndvi', 'evi', 'savi', 'ndwi5', 'ndwi6', 'area_ndvi'])
                                                 .set('doy', curdoy)
                                                 .set('year', curyear)
                                                 .set('system:time_start', curdate),
                                               brdfTemp.first()));
  ...
  function sumZonalBrdf(image) { 
    ...
    // ORDER is important, must correspond to reducers above
    var output = image2
    .select(['doy', 'year', 'ndvi', 'savi', 'evi', 'ndwi5', 'ndwi6', 'ndvi'], //extra ndvi for count pixels 
            ['doy', 'year', 'ndvi', 'savi', 'evi', 'ndwi5', 'ndwi6', 'tocount'])
    // .select(['doy', 'year', 'ndvi', 'savi', 'evi', 'ndwi5', 'ndwi6', 'area_ndvi', 'ndvi'], //extra ndvi for count pixels 
    //         ['doy', 'year', 'ndvi', 'savi', 'evi', 'ndwi5', 'ndwi6', 'area_ndvi', 'tocount'])
    .reduceRegions({
             collection: woredas,
             reducer: reducers,
  ...
  // Flatten the results for export
  var brdfNamesOld = ['NewPCODE', 'R_NAME', 'Z_NAME', 'W_NAME', 'yearmean', 'mean', 'ndvimean', 'savimean', 'evimean', 'ndwi5mean', 'ndwi6mean', 'good_pixelscount', 'total_pixelscount', ];
  var brdfNamesNew = ['NewPCODE', 'R_NAME', 'Z_NAME', 'W_NAME', 'year', 'doy', 'ndvi', 'savi', 'evi', 'ndwi5', 'ndwi6', 'pixels_ndvi', 'pixels_total'];
  // var brdfNamesOld = ['NewPCODE', 'R_NAME', 'Z_NAME', 'W_NAME', 'yearmean', 'mean', 'ndvimean', 'savimean', 'evimean', 'ndwi5mean', 'ndwi6mean', 'good_pixelscount', 'total_pixelscount', 'area_ndvisum', 'area_woreda'];
  // var brdfNamesNew = ['NewPCODE', 'R_NAME', 'Z_NAME', 'W_NAME', 'year', 'doy', 'ndvi', 'savi', 'evi', 'ndwi5', 'ndwi6', 'pixels_ndvi', 'pixels_total', 'area_ndvi', 'area_woreda'];
...
function exportToDrive(){
  ...
  Export.table.toDrive({
    ...
    selectors: ['NewPCODE', 'R_NAME', 'Z_NAME', 'W_NAME', 'year', 'doy', 'lst_day', 'lst_night', 'lst_mean', 'pixels_lstd', 'pixels_lstn', 'pixels_lstm', 'pixels_total']
    //selectors: ['NewPCODE', 'R_NAME', 'Z_NAME', 'W_NAME', 'year', 'doy', 'lst_day', 'lst_night', 'lst_mean', 'pixels_lstd', 'pixels_lstn', 'pixels_lstm', 'pixels_total', 'area_lstd', 'area_lstn', 'area_lstm', 'area_woreda']
  });
  Export.table.toDrive({
    collection: brdfFlat, 
    description: brdfFilename,
    selectors: ['NewPCODE', 'R_NAME', 'Z_NAME', 'W_NAME', 'year', 'doy', 'ndvi', 'savi', 'evi', 'ndwi5', 'ndwi6', 'pixels_ndvi', 'pixels_total']
    //selectors: ['NewPCODE', 'R_NAME', 'Z_NAME', 'W_NAME', 'year', 'doy', 'ndvi', 'savi', 'evi', 'ndwi5', 'ndwi6', 'pixels_ndvi', 'pixels_total', 'area_ndvi', 'area_woreda']
 });
...
function initializeWidgets() {
  ...
  var startDateLabel = ui.Label({
    value: 'Start Date for Summary (YYYY-MM-DD). For this script, the earliest start date is 2001-06-26 for LST data.',
  ...
  var calcButtonLabel = ui.Label({
    value: '2. Calculate environmental variables for selected dates. This step will take several seconds, please be patient.',
    style: {fontWeight: 'bold'}
  });
...
function createResultsPanel(userStartDate, userEndDate){
    ...
    var dateLabel = ui.Label({value: 'Optional, for VISUALIZATION of layers on the map: ' + 
...
function init() {
  ...
  var codeLink = ui.Label(
    ...
    {},
    'https://code.earthengine.google.com/d838dd0a6eb8a0fe8956f03a07ec6a67');
  ...
  var descriptionText = 
  '1. Enter a date range. Please request only a few months at one time, otherwise it may time out.';
